...
struct BaseGridMesh 
{
	std::vector<Vec3> basePositions; // only really need x,z intially. y is done by individual chunk heightmap
	std::vector<Vec2> baseUVs;
	std::vector<uint32_t> baseIndices;
};

BaseGridMesh GenerateMesh(int size) // size is number of vertices along one edge (size x size) 
{
	BaseGridMesh mesh;
