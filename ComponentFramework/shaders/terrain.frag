...
float ShadowValue(float shadow);

vec4 ClusterLightsColour(vec4 ktColour , vec3 eyeDir);
...
void main() { 
	...
	vec3 eyeDir = -normalize(vec3(fragViewPos));
	...
	float specLuminance = dot(specularC.rgb, LumVec );
	float specScale = min(0.5, 1.0 / max(specLuminance, 0.0001));
	specularC *= specScale;	

	vec4 diffC = diff * kd;
	vec4 phongContrib = (diffC + specularC) * gird_color;	
	float phongLuminance = dot(phongContrib.rgb, LumVec);
	float phongScale = min(0.8, 1.0 / max(phongLuminance, 0.0001));
	phongContrib *= phongScale;
	
	phongResult += shadow * phongContrib;

	vec4 ClusterColor =  ClusterLightsColour(gird_color, eyeDir);
	float ClusterLuminance = dot(ClusterColor.rgb, LumVec);
	float ClusterScale = min(0.8, 1.0 / max(ClusterLuminance, 0.0001));
	ClusterColor *= ClusterScale;

	phongResult += ClusterColor;
...
// params form a push and ubo
// gridScale
// fwidth(line) * 1.5; line width
// edgeColor
// edgeStrength
