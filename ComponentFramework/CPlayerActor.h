#pragma once
#include "CActor.h"
#include "CoreStructs.h"

class Renderer;

class CPlayerActor :
	public CActor
{
	// TODO: creationg and up dating of UBOs for camera data
	// thought maybe there should 
	// be a ubo component
	MATH::Matrix4 viewMatrix;
	MATH::Matrix4 projectionMatrix;

	std::vector<BufferMemory> cameraUBO;
	Renderer* render;
	bool uboNeedsUpdate = false;

public:
	CPlayerActor(Ref<Component> parent_, Renderer* render_) : CActor(parent_), render(render_) {}

	void UpdateProjectionMatrix(const float& FOVY, const float& aspectRatio, const float& nearClip, const float& farClip);
	void UpdateViewMatrix();
	virtual bool OnCreate() override;
	virtual void OnDestroy() override;
	virtual void Update(const float dt) override {}
	MATH::Matrix4 GetViewMatrix() const { return viewMatrix; }
	MATH::Matrix4 GetProjectionMatrix() const { return projectionMatrix; }

	void SetCameraUBO(const std::vector<BufferMemory>& ubo_) { cameraUBO = ubo_; }
	std::vector<BufferMemory> GetCameraUBO() { return cameraUBO; }

	CameraData GetCamDataUBO();
	bool IsUBOOutDated() const { return uboNeedsUpdate; }
	void UpdateUBO(uint32_t uboindex);

private:
	// input state variables
	bool moveForwardPressed = false;
	bool moveBackwardPressed = false;
	bool moveLeftPressed = false;
	bool moveRightPressed = false;

	//bool jumpPressed = false;
	bool crouchPressed = false;
	bool sprintPressed = false;
public:
	// movement functions
	void moveForward(bool move) { moveForwardPressed = move; }
	void moveBackward(bool move) { moveBackwardPressed = move; }
	void moveLeft(bool move) { moveLeftPressed = move; }
	void moveRight(bool move) { moveRightPressed = move; }
	//void jump(bool move) { jumpPressed = move; }
	void crouch(bool move) { crouchPressed = move; }
	void sprint(bool move) { sprintPressed = move; }

	void Jump(); // to be implemented when physics is added

};

